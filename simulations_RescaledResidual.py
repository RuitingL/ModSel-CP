...
import mtds_func_rescale_residual as RR
...
def train_model_feature_proj_ridge(M, X_tr, Y_tr, d_proj_portion, lamda):
    ...
    n = len(Y_tr)
    n1 = math.ceil(n/2); n2 = n-n1
    X_p = X_tr[:n1, :]; Y_p = Y_tr[:n1]
    X_v = X_tr[n1:, :]; Y_v = Y_tr[n1:]
    Mmodels = []
    ...
    for m in range(M):
        ...
        modl = Ridge(alpha=lamda).fit(X_tmp, Y_p)
        new_modl = feature_projection_pred_with_intercept(modl.coef_, modl.intercept_, ind_sub)
        
        # train the variance estimation
        prd = X_v[:,ind_sub]@modl.coef_ + modl.intercept_
        Z = np.abs(Y_v - prd)
        model_sigma = RandomForestRegressor()
        model_sigma.fit(X_v, Z)
        
        Mmodels.append([new_modl,model_sigma])
    return Mmodels
...
# experiments
def Mmodels_length_rescaleRes(Mmodels, X_cal, Y_cal, x_test, alpha):
    ...
    for m in range(M):
        mdl = Mmodels[m]
        S[m] = np.sort((np.abs(Y_cal - mdl[0].predict(X_cal))/mdl[1].predict(X_cal)))[k-1]
        S[m] = S[m]*mdl[1].predict(x_test)
    
    length = 2*S
    return length

def experiment_rr(distribution_type, N_rep, n_tr, n_cal, M, alpha, d_proj_portion, split_portion, lamda):
    ...
    for t in range(N_rep):
        ...
        coverE, lengthE, _, _, _ = RR.ModSel_rescaleRes(Mmodels, X_cal, Y_cal, x_test, y_test, alpha)
        coverL, lengthL, _, _ = RR.ModSelLOO_rescaleRes(Mmodels, X_cal, Y_cal, x_test, y_test, alpha)
        coverEFCP, lengthEFCP, _ = RR.YKbaseline_rescaleRes(Mmodels, X_cal, Y_cal, x_test, y_test, alpha)
        coverVFCP, lengthVFCP, _ = RR.YKsplit_rescaleRes(Mmodels, X_cal, Y_cal, x_test, y_test, 
                                                         alpha, split_portion)
        # if YK_adj will return infinity, just don't run it (replace with cov=1, length=inf later)
        if til_alpha <= 0:
            coverEFCP_adj = 0; lengthEFCP_adj = 0
        else:
            coverEFCP_adj, lengthEFCP_adj, _ = RR.YK_adj_rescaleRes(Mmodels, X_cal, Y_cal, x_test, y_test, alpha)
        ...
        benchL[:,t] = Mmodels_length_rescaleRes(Mmodels, X_cal, Y_cal, x_test, alpha)
...
for distribution_type in DISTR:
    for n_cal in N_cal:
        ...
        for M in M_size:
            cov, leng, min_single_md_len = experiment_rr(distribution_type,N_rep, n_tr, 
        ...
        filename = f"RescaleResidual_{distribution_type}_results_of_n_{n_cal}.csv"
...
for distribution_type in DISTR:
    for M in M_size:
        ...
        # define the file name
        filename = f"RescaleResidual_{distribution_type}_results_of_M_{M}.csv"
        # Save the DataFrame to a CSV file
        results_df.to_csv(filename, index=False)
